...
func InitWGPULibrary() {
	initLibOnce.Do(func() {
		...
		// 3. ~/.twlogaian/lib/ & ~/.twsla/lib/
		if home, err := os.UserHomeDir(); err == nil {
			candidates = append(candidates, filepath.Join(home, ".twlogaian", "lib", libName))
			candidates = append(candidates, filepath.Join(home, ".twsla", "lib", libName))
			if runtime.GOOS == "windows" {
				candidates = append(candidates, filepath.Join(home, ".twlogaian", "lib", "libwgpu_native.dll"))
