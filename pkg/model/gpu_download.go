...
// DefaultLibDir returns the primary directory where native libraries are stored (~/.twlogaian/lib)
func DefaultLibDir() string {
	if dir, err := os.UserHomeDir(); err == nil && dir != "" {
		return filepath.Join(dir, ".twlogaian", "lib")
	}
	if u, err := user.Current(); err == nil && u.HomeDir != "" {
		return filepath.Join(u.HomeDir, ".twlogaian", "lib")
	}
	return "twlogaian-lib"
}
...
func DownloadWGPULibrary(ctx context.Context, libDir string, progress func(downloaded, total int64)) (string, error) {
	...
	req.Header.Set("User-Agent", "twlogaian-downloader/1.0")
