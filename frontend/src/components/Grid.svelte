<script lang="ts">
  ...
  // Svelte 5 のレガシーリアクティブステートメント
  // 依存プロパティを列挙し、変更があればグリッドを更新する
  $: {
    const _deps = [node, data, columns, search, sort, pagination, language, width, height, autoWidth, fixedHeader, style, className, resizable, server, from];
    if (node) {
      updateGrid();
    }
  }
  ...
  function updateGrid() {
    ...
    if (!instance) {
      ...
      instance.updateConfig(config).forceRender();
    }
  }
